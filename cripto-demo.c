...
cryptoapi_demo(void)
{
	...
	sk.tfm = tfm;
	sk.req = req;
	sg_init_one(&sk.sg, input, DATA_SIZE);
    ...
    if (ret)
        goto out;
	printk(KERN_ERR PFX "ANTES: "); hexdump(encrypted, 2);//data_size ultimo parametro
	 printk(KERN_ERR PFX "DPS: "); hexdump(decrypted, 2);
	skcipher_request_set_crypt(req, &sg[0], &sg[1], DATA_SIZE, iv);
	crypto_skcipher_encrypt(req);
	skcipher_request_set_crypt(req, &sg[1], &sg[2], DATA_SIZE, iv);
	crypto_skcipher_decrypt(req);
	printk(KERN_ERR PFX "IN: "); hexdump(input, 2);
        printk(KERN_ERR PFX "ANTES: "); hexdump(encrypted, 2);//data_size ultimo parametro
	 printk(KERN_ERR PFX "DPS: "); hexdump(decrypted, 2);
